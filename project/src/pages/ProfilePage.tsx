...
export const ProfilePage: React.FC = () => {
  ...
  const handleAddPet = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setNewPet({
        name: '', breed: '', age: '', price: '', description: '', category: '', location: '', status: 'available',
      });
      setImageFile(null);
      setSelectedVaccines([]);
      setVaccineInput('');
                ...
                <button
                  ...
                  className={`py-5 px-2 border-b-4 font-semibold text-base flex items-center space-x-2 transition-colors ${
                    activeTab === tab.id
                      ...
                      : 'border-transparent text-gray-500 hover:text-primary-600 hover:border-primary-200 bg-white'
                  }`}
                >
        ...
        <div className="bg-white rounded-2xl shadow p-10">
                    ...
                    {userPets.slice(0, 3).map((pet) => (
                      <div key={pet.id} className="flex items-center space-x-3 mb-3 last:mb-0">
                        ...
                        <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                          ...
                          pet.status === 'sold' ? 'bg-gray-100 text-gray-800' :
                          'bg-yellow-100 text-yellow-800'
                        }`}>
                          {pet.status === 'available' ? 'Disponível' : pet.status === 'sold' ? 'Vendido' : 'Pendente'}
                        </span>
                    ...
                    {userServices.slice(0, 3).map((service) => (
                      <div key={service.id} className="flex items-center space-x-3 mb-3 last:mb-0">
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-sm text-gray-600">${service.price_from} - ${service.price_to}</p>
                        </div>
                        <span className={`px-2 py-1 rounded-full text-xs font-medium ${
          ...
          {activeTab === 'pets' && (
            <div>
              ...
              {showAddPetForm && (
                <form onSubmit={handleAddPet} className="bg-white rounded-2xl shadow-sm p-6 mb-8 grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2 mb-3">
                      {availableVaccines.map(vac => (
                        <button
                          type="button"
                          key={vac.id}
                          className={`px-2 py-1 rounded border ${selectedVaccines.includes(vac.id) ? 'bg-primary-500 text-white' : 'bg-white text-gray-700'}`}
                          onClick={() => setSelectedVaccines(selectedVaccines.includes(vac.id) ? selectedVaccines.filter(id => id !== vac.id) : [...selectedVaccines, vac.id])}
                        >
                          {vac.name}
                        </button>
                      ))}
                    ...
                    <div className="flex mb-3">
                      <input className="flex-1 px-3 py-2 border border-gray-300 rounded-l-lg" placeholder="Outra vacina" value={vaccineInput} onChange={e => setVaccineInput(e.target.value)} />
                      <button type="button" className="px-3 py-2 bg-primary-500 text-white rounded-r-lg" onClick={() => { if (vaccineInput && !selectedVaccines.includes(vaccineInput)) { setSelectedVaccines([...selectedVaccines, vaccineInput]); setVaccineInput(''); } }}>Adicionar</button>
                    </div>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Preço</label>
                    <input required type="number" min="0" className="w-full px-3 py-2 border border-gray-300 rounded-lg mb-3" value={newPet.price} onChange={e => setNewPet({ ...newPet, price: e.target.value })} />
              ...
              {userPets.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {userPets.map((pet) => (
                    <div key={pet.id} className="border border-gray-200 rounded-lg overflow-hidden hover:shadow-lg transition-shadow">
                      <img 
                        src={pet.image_url} 
                        alt={pet.name}
                      ...
                      <div className="p-4">
                        ...
                        <p className="text-lg font-bold text-primary-600 mb-3">${pet.price}</p>
